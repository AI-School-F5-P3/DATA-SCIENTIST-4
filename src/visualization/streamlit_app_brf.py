...
def load_model_and_scaler():
    try:
        ...
        model_path = os.path.join(project_root, 'models', 'balanced_random_forest', 'model_balanced_random_forest.pkl')
        scaler_path = os.path.join(project_root, 'models', 'balanced_random_forest', 'scaler_balanced_random_forest.pkl')
        
        model = joblib.load(model_path)
        scaler = joblib.load(scaler_path)
        return model, scaler
    except Exception as e:
        st.error(f"Error loading the model and scaler: {str(e)}")
        raise
...
try:
    ...
    st.error(f"Could not load the model or scaler: {str(e)}")
    st.stop()
