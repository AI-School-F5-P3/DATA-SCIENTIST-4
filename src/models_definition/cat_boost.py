...
def cross_validation_evaluate_model(X_train, y_train, model, cv=5):
    try:
        ...
        sample_weight = compute_sample_weight(class_weight_dict, y_train)
        cv_scores = cross_val_score(model, X_train, y_train, cv=cv, scoring='accuracy', n_jobs=-1, fit_params={'sample_weight': sample_weight})
...
def train_and_evaluate_model(X_train, X_test, y_train, y_test, class_weight_dict):
    try:
        ...
        base_model = CatBoostClassifier(random_state=42)
...
if __name__ == "__main__":
    ...
    best_model.save_model(f'{model_dir}/model_{model_name.lower().replace(" ", "_")}.cbm')
